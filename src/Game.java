...
public class Game {
	...
	GameThread[] threads = new GameThread[coreCount];
	double[] fitnessSums = new double[GENERATION_SIZE + 1];
	double[] fitnesses = new double[GENERATION_SIZE];
	AtomicInteger snakeIndex = new AtomicInteger(0);
	Random rand = new Random();
	int generationCount = 0;
//	int topIndex = -1;
	AtomicInteger topFood = new AtomicInteger(0);
	Graph graphAvg;
	Graph graphTop;
	final int layer1 = 12;
	final int layer2 = 10;
...
//		initRandomWieghts();
		...
		while (true) {
			...
			System.out.println("Best Snake Eaten: " + ((double) topFood.get() / ((double) X * (double) Y)) * 100
					+ "% of the food \n");
...
//			topIndex = -1;
			...
			setWeights();
//			weights = weights2;
			snakeIndex = new AtomicInteger(0);
			topFood = new AtomicInteger(0);
			generationCount++;
...
//		void setSnakeCount(int snakeCount) {
//			this.snakeCount = snakeCount;
//		}
		@Override
		public void run() {
			snakeCount = snakeIndex.getAndIncrement();
			while (snakeCount < GENERATION_SIZE) {
				...
				lastFoodStep = 0;
				alive = true;
				System.out.println(Thread.currentThread().getName()+" "+snakeCount);
				for (boolean[] b : taken) {
					Arrays.fill(b, false);
				}
				while (alive) {
				...
				topFood = new AtomicInteger(Math.max(topFood.get(), scoreFood));
				snakeCount = snakeIndex.getAndIncrement();
//				for (int i = 0; i < threads.length; i++) {
...
//		updateOutput();
			...
			double value = Math.max(nodes[nodes.length - 1][0], Math.max(nodes[nodes.length - 1][1],
					Math.max(nodes[nodes.length - 1][2], nodes[nodes.length - 1][3])));
